...
/* int2string converts an integer into a string and writes it in the
   ...
   characters). The return value is the count of characters written to s */
int int2string(int i, char *s)
{
...
/* It's not safe to use strcpy on overlapping strings, so here's a slower but
 * overlapped-safe version */
char* overlappedStrcpy(char *dst, const char *src)
{
